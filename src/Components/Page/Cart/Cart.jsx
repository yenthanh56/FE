...
const Cart = (props) => {
	...
	const onSubmitPayHandler = (e) => {
		...
		if (!address) {
			...
			addessRef.current.focus();
			return;
		}
		if (address.length <= 10) {
			toast.error("Bạn Phải Nhập Hơn 10 Ký Tự");
			addessRef.current.focus();
		...
		if (!city) {
			toast.error("Bạn Chưa Nhập Thành Phố");
		...
		if (!district) {
			toast.error("Bạn Chưa Nhập Quận Huyện");
			districtRef.current.focus();
			return;
		}
		if (!ward) {
			toast.error("Bạn Chưa Nhập Phường Xã");
		...
		if (phone !== "") {
			...
			if (regexNumber.test(phone) === false) {
				toast.error("Số điện thoại của bạn không đúng định dạng!");
				phoneRef.current.focus();
				return;
			}
		}
			...
			{!!totalPrice() && products.length > 0 && (
				<div className={cx("cart__userinformation")}>
					...
					<div className={cx("cart__userinformation__address")}>
						<div
							className={cx(
								"cart__userinformation__address__address"
							)}
						>
							<label>Nhập Địa Chỉ Nơi Giao:</label>
							<input
								type="text"
								value={address}
						...
						<div
							className={cx(
								"cart__userinformation__address__city"
							)}
						>
							<label>Nhập Tỉnh/Thành Phố:</label>
							<input
								type="text"
								value={city}
						...
						<div
							className={cx(
								"cart__userinformation__address__district"
							)}
						>
						...
						<div
							className={cx(
								"cart__userinformation__address__ward"
							)}
						>
							<label htmlFor="">Nhập Phường/Xã:</label>
							<input
								type="text"
								value={ward}
						...
						<div
							className={cx(
								"cart__userinformation__address__phone"
							)}
						>
